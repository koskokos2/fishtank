...
- **Pixel art, no external asset pipeline.** The reef backdrop is generated procedurally in code (`src/backdrop.ts`), baked to an offscreen canvas, and handed to Kaplay via `loadSprite`. The fish, nautilus, octopus, and jellyfish are native transparent pixel-art atlases (`128px` cells) embedded as base64 data URLs; fish swim sheets are synthesized from static atlas cells at load, while the nautilus, octopus, and jellyfish ship real frame atlases baked at build and swapped live. No PNG files are loaded at runtime — every image is code or an embedded data URL.
...
src/
  ...
  cephalopod.ts # Octopus (baked pose frames — an idle arm-sway loop + swim/turn poses — swapped by a crawl/swim state machine, benthic) + nautilus (own runtime atlas — fixed shell + baked tentacle wave/siphon/jet layers, jet-cruise motion) + jellyfish (16-pose atlas swapped by the "pulse" machine: pulse cycle synced to thrust, streaming glides, hover variety, turn rolls, rare startle); shared spawn/motion scaffolding
...
tools/
  ...
  gen-nautilus-atlas.ts # one-off: build src/nautilusAtlas.ts (+ art/nautilus-atlas-128.png) from art/nautilus-components-transparent.png — four identity-stable pieces (shell/body, tentacle crown, siphon, jet plume) deformed into the runtime frames
...
- **Generated source cells can lie.** A fixed source grid is a review aid, not proof that every sprite is contained in its nominal cell. If a prop crosses a source boundary or pollutes a neighbour, fix extraction with frame-specific regions/connected-component logic and assertions; do not blindly bbox the whole cell. If a transparent pass destroyed intentional key-adjacent colour, recover only that frame from the chroma source with background-connected matte removal, safe recolour, contact-band protection, and fleck validation.
- **Every creature is a pixel-art atlas; animation is all whole-pixel.** The fish are a transparent atlas embedded as a base64 data URL (`src/fishAtlas.ts` from `art/fish-atlas-128.png`), one native 128px cell per fish. A new fish = a new 128px cell + an entry in `FISH_KINDS` (name + habitat level); regenerate with `tools/gen-fish-atlas.ts`. The fish frames are static, so a tail-swish "swim" is synthesized at bake by shearing the rear of the body in whole-pixel steps (`fishbake.ts`) — keeping it on the integer pixel grid — played at a speed tied to swim speed. The nautilus has its own runtime atlas (`src/nautilusAtlas.ts` from `art/nautilus-components-transparent.png`, built by `tools/gen-nautilus-atlas.ts`): four identity-stable source pieces (shell/body, tentacle crown, siphon, jet plume) baked into a 16-col × 4-row frame atlas — a fixed shell row, a continuously travelling tentacle wave, a siphon retract→extend band, and a one-shot jet plume. The shell/eye/attachment points stay rigid; the jet machine in `cephalopod.ts` swaps each layer's frame independently (`NAUTILUS_*_START`).
