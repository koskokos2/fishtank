...
//
// ?res=1|2|6 overrides the authored density per launch, so one build serves weak
// devices (a Pi renders 4-9x fewer pixels) and 4K displays (res=6 → 3840x2160) —
// every consumer reads RES at module init, before the scene exists. Headless
// tools have no location and bake at the authored default.
const AUTHORED_RES = 3;
const queryRes =
  typeof location === "undefined"
    ? NaN
    : Number(new URLSearchParams(location.search).get("res"));
export const RES = [1, 2, 3, 6].includes(queryRes) ? queryRes : AUTHORED_RES;
